...
def idioma():
	...
	system("echo 'LANG=" + locale + "' > /etc/locale.conf")
...
def host():
	...
	system("echo " + hostname + " > /etc/hots")
	info("Modificando el archivo /etc/hosts")
	system('echo -e "127.0.0.1\tlocalhost" >> /etc/hosts')
	system('echo -e "::1\tlocalhost" >> /etc/hosts')
	system('echo -e "127.0.0.1\t' + hostname + '" >> /etc/hosts')
...
system("grub-install --target=x86_64 --efi-directory=/efi --bootloader-id=GRUB")
